    ...
    public void testSeqNoCASLinearizability() {
        ...
        List<Partition> partitions =
            IntStream.range(0, numberOfKeys)
                .mapToObj(i -> {
                    execute("insert into t (id, x) values (?, ?)", new Object[]{i, "value " + i});
                    execute("select _primary_term, _seq_no from t where id = ?", new Object[]{i});
                    ...
                    for (int i = 0; i < numberOfUpdates; ++i) {
                        ...
                        try {
                            ...
                            if(response.rowCount() == 1) {
                                ...
                                execute("select _primary_term, _seq_no from t where id = ?",
                                        new Object[]{partition.id});
                                long primaryTerm = (long) response.rows()[0][0];
                                long seqNo = (long) response.rows()[0][1];
